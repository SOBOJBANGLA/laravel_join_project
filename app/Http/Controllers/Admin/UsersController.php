...
class UsersController extends Controller
{
    ...
    public function agent_users()
    {
        ...
        return view('admin.agent.user', compact('agents'));
    }

    public function agent_users_save(Request $request)
    {
        $new_agent = new Agent();
        $new_agent->firstname = $request->firstname;
        $new_agent->lastname = $request->lastname;
        $new_agent->username = $request->username;
        $new_agent->email = $request->email;
        $new_agent->password = Hash::make($request->password);
        $new_agent->save();


        $ag_data = Agent::where('id', $new_agent->id)->first();
        $ag_data->agent_id = Str::random(5) . $new_agent->id . Str::random(5);
        $ag_data->save();

        return back()->with('success', 'Agent Successfully Created');
    }

    public function agent_users_update(Request $request)
    {
        $new_agent = Agent::where('id', $request->agent_edit_id)->first();
        $new_agent->firstname = $request->firstname;
        $new_agent->lastname = $request->lastname;
        $new_agent->username = $request->username;
        $new_agent->email = $request->email;
    ...
    {
        $user = Agent::where('id', $request->agent_delete_id)->first();
        $user->delete();
    ...
    {
        $agents = Agent::all();
        $funds = fund_transfer::where('send_to', 'agent')->where('send_by', 'admin')->with('agent')->paginate(20);

        return view('admin.agent.agentFund', compact('funds', 'agents'));
    }

    public function agent_fund_save(Request $request)
    {
        $transfer = new fund_transfer();
        $transfer->agent_id = $request->agent_id;
        $transfer->amount = $request->amount;
        $transfer->send_to = 'agent';
        $transfer->send_by = 'admin';
        $transfer->status = 1;
        $transfer->save();
        ...
        return back()->with('success', 'Fund Transfer Send Successfully');
