...
class HomeController extends Controller
{
    ...
    {
        $transfers = fund_transfer::where('user_id', Auth::user()->id)
            ->orderBy('id', 'desc')
            ->paginate(10);
        return view($this->theme . 'user.transaction.fundTransfer', compact('transfers'));
    }


    public function fund_transfer_save(Request $request)
    {
        $agent = Agent::where('agent_id', Auth::user()->agent_id)->first();
        if (!$agent) {
